      ...
      <div className="bg-white rounded-xl shadow-2xl w-96 flex flex-col">
        ...
        <div className="px-6 py-4 space-y-3">
          <div className="bg-gray-50 rounded-lg p-3 text-sm">
            ...
            <div className="flex justify-between text-gray-600 mb-1">
              <span>Party</span><span>{invoice.partyName || '—'}</span>
            </div>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-3 border-b border-gray-200 bg-white shrink-0 flex items-center justify-between">
        <h1 className="text-base font-semibold text-gray-800">Sale Invoices</h1>
      ...
      {invoices.length === 0 ? (
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 mb-2">No Sale Invoices</h2>
            <p className="text-gray-500 text-sm mb-6">Create your first sale invoice to get started.</p>
            <button onClick={() => onNavigate('sale-invoice-new')}
        ...
        <div className="flex-1 overflow-auto bg-gray-50 p-5">
          <div className="grid grid-cols-3 gap-4 mb-4">
            {[
              { label: 'Total Invoiced', value: totals.total, color: 'text-gray-800' },
              { label: 'Amount Received', value: totals.paid, color: 'text-green-600' },
              { label: 'Outstanding', value: totals.balance, color: 'text-red-600' },
            ].map(s => (
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['Invoice #', 'Party', 'Date', 'Due Date', 'Total', 'Paid', 'Balance', 'Status', 'Action'].map(h => (
                    ...
                    <td className="px-4 py-2.5 text-center">
                      {inv.status !== 'paid' && (
                        <button onClick={() => setPayInvoice(inv)}
                          ...
                          <BanknotesIcon className="w-3.5 h-3.5" /> Receive
                        </button>
                      )}
                    </td>
                  </tr>
